...
> **Figura 7.1** — Diagrama do pipeline de produção de um carro em *Forza Motorsport*, mostrando as etapas sequenciais: scan laser → retopologia → higiene (normais, transformações, pivô) → UV unwrapping → baking → texturização → LODs → exportação e verificação no motor. Cada caixa tem um critério de aprovação antes de avançar.
>
> **Fonte da imagem:** Turn 10 Studios — *Forza Motorsport 6* (2015). Imagens oficiais disponíveis em: `https://news.xbox.com/en-us/category/forza/` (Xbox Wire, acesso público). Detalhes do pipeline: GDC 2016 — *"The Art of Forza Motorsport 6: Apex"*, Christian Ammann, Turn 10 Studios (GDC Vault).
...
- *What Is Texture Baking?* e *Baking Guide* (materiais de apoio da disciplina) — introduzem o processo de transferência de detalhe entre a malha de alta e a de baixa densidade, que a preparação deste capítulo torna possível e que a Parte V (Capítulos 15 e 16) desenvolverá.
