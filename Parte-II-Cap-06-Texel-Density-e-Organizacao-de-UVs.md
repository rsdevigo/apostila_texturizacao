...
Uma equipe monta a cena de um pátio medieval e nota, ao percorrê-la, que algo destoa: os barris empilhados num canto parecem nítidos, mas o portão de madeira ao fundo, embora próximo, aparece visivelmente borrado. A primeira suspeita recai sobre a resolução das texturas, mas a investigação revela que ambos os *assets* usam mapas de mesma resolução. O problema, descoberto ao aplicar a mesma textura de verificação aos dois objetos, está na densidade de texels: os quadrados do xadrez aparecem pequenos nos barris e grandes no portão. O portão, sendo um objeto fisicamente muito maior que um barril, recebeu a mesma resolução de mapa, mas suas ilhas UV precisaram cobrir uma superfície bem maior — resultado: densidade muito mais baixa e textura borrada.

A correção segue o método do capítulo. A equipe primeiro define formalmente um valor-alvo de densidade para os *assets* de cenário àquela distância e mede os dois objetos contra ele. Os barris estão acima do alvo; o portão, muito abaixo. Para o portão, em vez de simplesmente inflar a resolução do mapa — o que encareceria a memória —, a equipe percebe que o portão é composto de tábuas verticais praticamente idênticas. Aplica-se, então, a sobreposição: as ilhas das tábuas repetidas são empilhadas para compartilhar a mesma textura, o que multiplica a densidade efetiva sem aumentar o tamanho do mapa. Onde a sobreposição não basta, eleva-se moderadamente a resolução até alcançar o alvo. Reaplicado o xadrez, os quadrados do portão e dos barris passam a ter o mesmo tamanho, e a cena recupera a coerência visual. O caso ilustra três lições conjugadas: que objetos de tamanhos físicos diferentes exigem ilhas dimensionadas para igualar a densidade, e não mapas de mesma resolução; que a textura de verificação é o instrumento que torna a inconsistência diagnosticável; e que a sobreposição de partes repetidas é, muitas vezes, a forma mais econômica de elevar a densidade.

> Figura: A cena do pátio antes e depois da correção — na primeira, o portão borrado ao lado dos barris nítidos; na segunda, ambos com o xadrez de quadrados equivalentes —, acompanhada do layout do portão mostrando as tábuas sobrepostas.
>
> **Screenshot sugerido:**
> Captura comparativa da cena com a textura de verificação aplicada, evidenciando os quadrados desiguais antes e equivalentes depois, com o editor UV do portão ao lado.
