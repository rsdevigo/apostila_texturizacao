...
Considere uma equipe encarregada de texturizar uma lata de refrigerante simples para um jogo. À primeira vista, um único método de projeção pareceria suficiente, dada a forma aproximadamente cilíndrica do objeto. A equipe começa, portanto, aplicando uma projeção cilíndrica ao corpo da lata, e o resultado é imediato e satisfatório: o rótulo se enrola perfeitamente ao redor do corpo, com os quadrados da textura de verificação distribuídos de modo uniforme em toda a parede cilíndrica. O problema, porém, aparece nas extremidades. A tampa superior, com seu anel e sua lingueta, e a base côncava recebem da projeção cilíndrica um esticamento severo: ali, a superfície se curva para dentro e deixa de acompanhar o cilindro, e os quadrados do xadrez se transformam em longos raios distorcidos que convergem para o centro.

A solução adotada pela equipe ilustra um princípio central do mapeamento: poucos objetos do mundo real correspondem a uma única forma primitiva. A lata foi tratada por partes. O corpo recebeu a projeção cilíndrica que já funcionava bem. A tampa e a base, sendo superfícies aproximadamente planas e circulares, foram separadas em ilhas próprias e receberam, cada uma, uma projeção planar vista de cima e de baixo — exatamente a projeção que melhor trata superfícies chatas. Ao reunir as três ilhas no layout, a equipe obteve um mapeamento uniforme em toda a lata, sem distorção em nenhuma região. O aprendizado é que as projeções não são soluções totais e mutuamente exclusivas, mas ferramentas a serem combinadas conforme a anatomia do objeto, decompondo a forma complexa em regiões que cada projeção sabe tratar bem.

> Figura: Sequência de três imagens da mesma lata com textura de verificação — a primeira com projeção cilíndrica única, mostrando a distorção radial na tampa e na base; a segunda destacando a separação em três ilhas (corpo, tampa, base); a terceira com o mapeamento final uniforme.
>
> **Screenshot sugerido:**
> Captura do editor UV exibindo as três ilhas da lata separadas e o modelo correspondente com o xadrez regular em toda a superfície.
